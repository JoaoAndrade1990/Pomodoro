import { Container } from "../../components/Container/Container";
import { MainTemplate } from "../../templates/MainTemplates/index";
import { GenericHtml } from '../../components/GenericHTML/index';
import { Heading } from "../../components/Heading/Heading.tsx";

export function NotFound() {
  return (
    <MainTemplate>
      <Container>
        <GenericHtml>
          <Heading>404 - Página não encontrada 🚀</Heading>
          <p>
            Ups! Parece que a página que está a tentar acessar não existe.
            Talvez ela tenha tirado férias, resolvido explorar o universo ou se
            perdido em algum lugar entre dois buracos negros. 🌌
          </p>

          <p>
            Mas calma, não está perdido no espaço (ainda). Dá pra voltar em
            segurança para a <a href="/">página principal</a> ou{" "}
            <a href="/history">para o histórico</a> — ou pode ficar por aqui e
            fingir que achou uma página secreta que só os exploradores mais
            fixes conseguem acessar. 🧭✨
          </p>

          <p>
            {" "}
            Se acha que esta página deveria existir (ou se quiser conversar sobre viagem no tempo e buracos de minhoca), é só entrar em
            contato. Caso contrário, use o menu para voltar ao mundo real.
          </p>

          <p>
            Enquanto isso, fica aqui uma reflexão: "Se uma página não existe na
            internet, será que ela existiu de verdade?" 🤔💭
          </p>
        </GenericHtml>
      </Container>
    </MainTemplate>
  );
}
